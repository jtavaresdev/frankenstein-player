...
namespace core {
    ...
    Song::Song(const std::string& title, Artist& artist, Album& album)
        : _title(title),
          _artist(std::make_shared<Artist>(artist)),
          _album(std::make_shared<Album>(album)) {};

    Song::Song(unsigned id, std::string title, unsigned artist_id)
        : Entity(id),
    ...
    void Song::setTitle(const std::string& title) {
        if (title.empty()) {
            throw std::invalid_argument(
                "Título da música não pode estar vazio");
        }
        _title = title;
    };
    ...
    std::string Song::toString() const {
        std::string info = "{Musica: " + _title
                           ...
                           + ", Duracao: " + std::to_string(getDuration())
    ...
    bool Song::operator==(const Entity& other) const {
        ...
        if (otherSong == nullptr) {
            throw std::invalid_argument(
                "Erro no casting: objeto não é do tipo Song");
        }

        if (otherSong->getId() == this->getId()
            && otherSong->_title == this->_title) {
            return true;
        }
        return false;
    };
