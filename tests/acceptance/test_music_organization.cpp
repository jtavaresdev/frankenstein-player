...
TEST_SUITE("HISTÓRIA DE USUÁRIO: Organização de Músicas") {
    ...
    std::unique_ptr<core::SongRepository> song_repo = repo_factory.createSongRepository();
    std::unique_ptr<core::ArtistRepository> artist_repo = repo_factory.createArtistRepository();
    std::unique_ptr<core::AlbumRepository> album_repo = repo_factory.createAlbumRepository();
    std::unique_ptr<core::UserRepository> user_repo = repo_factory.createUserRepository();


    void checkSongInDatabase(const MediaFixture::SongTestMock &song, const core::User &user) {
    ...
    TEST_CASE("CT-AC-01: Adicionar e organizar um album") {
        SUBCASE("Organizar um álbum") {
            ...
            auto song_mock1 = media.getSongTestMock("Short_Song_Test_The_Testers");
            auto song_mock2 = media.getSongTestMock("Medium_Song_Test_The_Testers");

            boost::filesystem::copy(song_mock1.path, user.getInputPath());
            boost::filesystem::copy(song_mock2.path, user.getInputPath());
            manager.update();

            std::string song1_path_registre = user.getInputPath() + song_mock1.artist + "/" + song_mock1.album + "/" + song_mock1.title + ".mp3";
            std::string song2_path_registre = user.getInputPath() + song_mock2.artist + "/" + song_mock2.album + "/" + song_mock2.title + ".mp3";
        ...
        SUBCASE("Organizar musicas de mais de um usuario") {
            ...
            const MediaFixture::SongTestMock song_mock1 = media.getSongTestMock("Short_Song_Test_The_Testers");
            const MediaFixture::SongTestMock song_mock2 = media.getSongTestMock("Short_Song_Examples_Example_Band");

            boost::filesystem::copy(song_mock1.path, user1.getInputPath());
            boost::filesystem::copy(song_mock2.path, user2.getInputPath());
            manager.update();

            std::string song1_path_registre = user1.getInputPath() + song_mock1.artist + "/" + song_mock1.album + "/" + song_mock1.title + ".mp3";
            std::string song2_path_registre = user2.getInputPath() + song_mock2.artist + "/" + song_mock2.album + "/" + song_mock2.title + ".mp3";

            CHECK(boost::filesystem::exists(song1_path_registre));
            CHECK(boost::filesystem::exists(song2_path_registre));
            checkSongInDatabase(song_mock1, user1);
            checkSongInDatabase(song_mock2, user2);
            CHECK(song_repo->findByTitleAndUser(song_mock1.title, user2).size() == 1);
        ...
        SUBCASE("Organizar música sem usuario") {
            ...
            std::string song1_path_registre = config.inputPublicPath() + song_mock1.artist + "/" + song_mock1.album + "/" + song_mock1.title + ".mp3";
            ...
            if (!song_mock1.artist.empty())
                ...
                FAIL_CHECK("Erro: song_mock1.artist está vazio, não foi possível verificar o artista.");
            if (!song_mock1.album.empty())
                CHECK(songs[0]->getAlbum()->getName() == song_mock1.album);
            else
                FAIL_CHECK("Erro: song_mock1.album está vazio, não foi possível verificar o álbum.");
    ...
    TEST_CASE("CT-AC-02: Adicionar e organizar uma musica") {
        SUBCASE("Organizar uma música") {
            ...
            const MediaFixture::SongTestMock song_mock = media.getSongTestMock("Short_Song_Test_The_Testers");

            boost::filesystem::copy(song_mock.path, user.getInputPath());

            manager.update();

            std::string song_path_registre = user.getInputPath() + song_mock.artist + "/" + song_mock.album + "/" + song_mock.title + ".mp3";
        ...
        SUBCASE("Organizar multiplas musicas") {
            ...
            const auto song_mock1 = media.getSongTestMock("Short_Song_Test_The_Testers");
            const auto song_mock2 = media.getSongTestMock("Medium_Song_Test_The_Testers");
        ...
        SUBCASE("Organizar música sem album") {
            ...
            const MediaFixture::SongTestMock song_mock = media.getSongTestMock("Song_Test_No_Album");

            boost::filesystem::copy(song_mock.path, user.getInputPath());
            manager.update();

            std::string song_path_registre = user.getInputPath() + song_mock.artist + "/Singles/" + song_mock.title + ".mp3";
            CHECK(boost::filesystem::exists(song_path_registre));
            checkSongInDatabase(song_mock, user);
        ...
        SUBCASE("Reorganizar após remoção de musicas") {
            ...
            CHECK(song_repo->findByTitleAndUser(song_mock.title, user).size() == 0);
        }
    }
    ...
    TEST_CASE("CT-AC-03: Adicionar e organizar musicas conforme autores") {
        SUBCASE("Organizar musicas de artistas diferentes") {
            ...
            const MediaFixture::SongTestMock song_mock = media.getSongTestMock("Short_Song_Examples_Example_Band");
            boost::filesystem::copy(song_mock.path, user.getInputPath());
            const MediaFixture::SongTestMock song_mock2 = media.getSongTestMock("Short_Song_Test_The_Testers");
            boost::filesystem::copy(song_mock2.path, user.getInputPath());
            manager.update();
            ...
            for (const auto& artist : artists) {
                if (artist->getName() == song_mock.artist) found_artist1 = true;
                if (artist->getName() == song_mock2.artist) found_artist2 = true;
            }
        ...
        SUBCASE("Organizar musicas de mesmo autor de usuários diferentes") {
            ...
            const MediaFixture::SongTestMock song_mock1 = media.getSongTestMock("Short_Song_Test_The_Testers");
            boost::filesystem::copy(song_mock1.path, user1.getInputPath());
            const MediaFixture::SongTestMock song_mock2 = media.getSongTestMock("Medium_Song_Test_The_Testers");
            boost::filesystem::copy(song_mock2.path, user2.getInputPath());
            manager.update();
