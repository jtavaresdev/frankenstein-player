...
private:
  ...
  std::vector<size_t> _indices_aleatory; /*!< @brief Índices embaralhados para
...
public:
    PlaybackQueue();
    PlaybackQueue(std::shared_ptr<User> current_user, const IPlayable &playable,
    ...
    */
    std::vector<std::shared_ptr<const Song>> getQueueView(size_t before,
                                                            size_t after) const;

    /**
    * @brief Obtém uma visualização de parte da fila, tamanho será menor que o
    * passado se o fim da fila for alcançado
    * @param start Índice inicial da visualização
    * @param count Número de músicas na visualização
    * @return Vetor de músicas na visualização
    */
    std::vector<std::shared_ptr<const Song>> getQueueSegment(size_t start,
                                                            size_t count) const;
